<script lang="ts">
	...
	export async function refresh() {
		...
		userDrips = await subgraphClient.getDripsBySender((await dripsClient.signer.getAddress()).toLowerCase());
		userSplits = await subgraphClient.getSplitsBySender((await dripsClient.signer.getAddress()).toLowerCase());
		updating = false;
	}

	$: Boolean(dripsClient) && refresh();
</script>
