...
function estimateHistoryItem(
	...
	const streamEstimates = historyItem.streams.map((receiver) => {
		const estimate = streamedByStream(window, receiver, senderId, historyItem, excludingSqueezes, nextHistoryItem);
...
function buildAssetConfigEstimates(
  ...
  */
	const totalEstimate = estimateAssetConfig(assetConfig, { from: 0, to: Number.MAX_SAFE_INTEGER }, userId);
...
export function estimateAccount(
	...
	return Object.fromEntries(
		account.assetConfigs.map((assetConfig) => [
			assetConfig.tokenAddress,
			buildAssetConfigEstimates(assetConfig, currentCycle, account.userId, excludingSqueezes)
		])
	);
}
