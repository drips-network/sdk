...
export type DripsReceiverConfig = {
	/** The UNIX timestamp when dripping should start. When setting a config, it must be greater than or equal to `0`. If set to `0`, the smart contract will use the timestamp when drips are configured. */
	...
	/** The amount per second being dripped. When setting a config, it must be in the smallest unit (e.g. Wei), greater than `0` and be multiplied by `10 ^ 18` (Utils.Constants.AMT_PER_SEC_MULTIPLIER). */
