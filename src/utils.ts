...
export const fromWei = (wei: BigNumberish) => {
  wei = BigNumber.isBigNumber(wei) ? wei : BigNumber.from(wei)
  return wei.div(constants.WeiPerEther)
}

export const toWei = (dai: BigNumberish) => {
  return utils.parseUnits(dai.toString())
}

export const toDAI = (wei: BigNumberish, frmt = 'pretty', roundTo: number) => {
...
export const getDripsWithdrawable = (config: DripsConfig) => {
  ...
  try {
    ...
    let withdrawable = eventBalance.sub(totalAmtPerSec.mul(currTimestamp - parseInt(config.timestamp)))
