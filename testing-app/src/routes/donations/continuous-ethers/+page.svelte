<script lang="ts">
  ...
  function generateStreamName() {
    const timestamp = new Date()
      ...
      .slice(0, 19)
      .replace(/[:-]/g, '');
    streamName = `Continuous Donation (Ethers) - ${timestamp}`;
  }
...
<h1>Continuous Donation (Ethers)</h1>

<div class="info-box">
  <strong>📝 About this operation:</strong><br />
  This page replicates the "Continuous Donation using Ethers" integration test. It
  will fetch an existing drip list, set up an Ethers wallet, send the continuous
...
<div class="form-container">
  ...
  >
    {#if $operationStatus.isRunning}
      🔄 Sending...
    {:else}
      🌊 Send Continuous Donation with Ethers
    {/if}
  </button>
</div>
