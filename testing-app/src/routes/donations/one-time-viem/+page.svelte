  ...
  async function sendOneTimeDonation() {
    try {
      ...
      const approveHash = await walletClient.writeContract({
        ...
        args: [addressDriverAddress, donationAmountBigInt],
      });
...
<h1>One-Time Donation (Viem)</h1>

<div class="info-box">
  <strong>📝 About this operation:</strong><br />
  This page replicates the "One-Time Donation using Viem" integration test. It will
  fetch an existing drip list, set up a Viem wallet client, send the donation, and
...
<div class="form-container">
  ...
  >
    {#if $operationStatus.isRunning}
      🔄 Sending...
    {:else}
      💰 Send One-Time Donation with Viem
    {/if}
  </button>
</div>
