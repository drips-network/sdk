...
// 	describe('create()', () => {
// 		it('should throw argumentError error when API URL is missing', async () => {
// 			let threw = false;
...
// 			assert.isTrue(
// 				toBN(configs![0].tokenAddress).eq(utils.getTokenAddressFromAssetId(assetConfigs[0].assetId))
// 			);
// 			assert(
...
// 			// Assert
// 			assert.isTrue(toBN(config!.id).eq(userAssetConfig.id));
// 			assert.isTrue(toBN(config!.tokenAddress).eq(utils.getTokenAddressFromAssetId(userAssetConfig.assetId)));
// 			assert(
