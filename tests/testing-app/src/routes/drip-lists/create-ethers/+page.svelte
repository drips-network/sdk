<script lang="ts">
  ...
  let dripListName = `Test Drip List (Ethers) - ${generateRandomId()}`;
  let dripListDescription = 'A test drip list created using Ethers adapter';
  let isVisible = true;
  ...
  async function createDripListWithEthers() {
    try {
      resetOperation();
      ...
      updateOperationStatus({
        progress: 100,
        isRunning: false,
        result: {dripListId, salt, ipfsHash, txHash: receipt.hash},
      });
    } catch (error) {
      const errorMessage =
...
<h1>Create Drip List (Ethers)</h1>

<div class="info-box">
  <strong>📝 About this operation:</strong><br />
  This page replicates the "Create Drip List using Ethers" integration test. It will
  set up an Ethers wallet/signer, define split receivers, create the drip list, upload
